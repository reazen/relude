...
module FunctorExtensions = (F: BsBastet.Interface.FUNCTOR) => {
  ...
  /**
   * Applies an argument of type `'a` to a functor of `'a => 'b` functions, to produce a functor of `'b`
   */
...
module FunctorInfix = (F: BsBastet.Interface.FUNCTOR) => {
  ...
  /**
   * Operator version of the `map` function, which has the function on the left, and functor on the right.
   */
  let (<$>) = F.map;

  /**
   * Operator version of the `flipMap` function, which has the functor on the left, and function on the right.
   */
  let (<#>) = FunctorExtensions.flipMap;

  /**
   * Operator version of voidRight, which replaces the values of the functor on the right with the value on the left.
   */
  let (<$) = FunctorExtensions.voidRight;

  /**
   * Operator version of voidLeft, which replaces the values of the functor on the left with the value on the right.
   */
  let ($>) = FunctorExtensions.voidLeft;

  /**
   * Operator version of flap, which takes a functor of functions `F.t('a => 'b)` on the left, and an `'a` on the right,
   * and applies the value `'a` to each function, producing a functor of `'b` (`F.t('b)`).
   */
