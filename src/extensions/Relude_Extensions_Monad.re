...
module MonadExtensions = (M: BsBastet.Interface.MONAD) => {
   ...
   * `map` with the function on the left, and `flatMap` is another commonly used name for `bind`.
   */
  ...
  /**
   * Creates a new monadic function by composing two monadic functions from left-to-right.
   */
  let composeKleisli:
    'a 'b 'c.
  ...
  /**
   * Creates a new monadic function by composing two monadic functions from right-to-left.
   */
  ...
  /**
   * Runs a monadic effect if the given monadic condition is evaluated to true.
   * If the condition is false, a pure unit value is returned with no effect being run.
   */
  let when_: (M.t(bool), M.t(unit)) => M.t(unit) = BsMonadExtensions.when_;

  /**
   * Runs a monadic effect if the given monadic condition is evaluated to false.
   * If the condition is true, a pure unit value is returned with no effect being run.
   */
...
module MonadInfix = (M: BsBastet.Interface.MONAD) => {
  ...
  /**
   * Operator version of applying a monadic function with the monad value on the left, and the function on the right.
   */
  let (>>=) = M.flat_map;

  /**
   * Operator version of applying a monadic function with the monad value on the right, and the function on the left.
   */
