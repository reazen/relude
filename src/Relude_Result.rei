...
let tapOk: ('a => unit, t('a, 'e)) => t('a, 'e);
let tapError: ('e => unit, t('a, 'e)) => t('a, 'e);
let apply: (t('a => 'b, 'e), t('a, 'e)) => t('b, 'e);
let map2: (('a, 'b) => 'c, t('a, 'x), t('b, 'x)) => t('c, 'x);
let map3: (('a, 'b, 'c) => 'd, t('a, 'x), t('b, 'x), t('c, 'x)) => t('d, 'x);
let map4:
...
let alt: (t('a, 'e), t('a, 'e)) => t('a, 'e);
let catchError: ('e => t('a, 'e), t('a, 'e)) => t('a, 'e);
let recover: ('a, t('a, 'e)) => t('a, 'e);
let getOrElse: ('a, t('a, 'e)) => 'a;
let fromOption: ('e, option('a)) => t('a, 'e);
let fromOptionLazy: (unit => 'e, option('a)) => t('a, 'e);
...
let triesAsString: (unit => 'a) => t('a, Js.String.t);
let toValidation: Belt.Result.t('a, 'b) => Relude_Validation.t('a, 'b);
let fromValidation: Relude_Validation.t('a, 'b) => Belt.Result.t('a, 'b);
let toValidationNel:
...
module WithError:
  (E : BsAbstract.Interface.TYPE) => {
                                       ...
                                       let indexOf:
                                         ((module BsAbstract.Interface.EQ with type t = 'a),
                                         'a, Foldable.t('a)) => option(int);
                                       let minBy:
                                         (('a, 'a) =>
                                          Relude_Extensions_Foldable.ordering,
                                         Foldable.t('a)) => option('a);
                                       let min:
                                       ...
                                       let max:
                                         ...
                                         Foldable.t('a)) => option('a);
                                       let countBy:
                                         ('a => bool, Foldable.t('a)) => int;
                                       let length: Foldable.t('a) => int;
                                       let forEach:
                                         ('a => unit, Foldable.t('a)) => unit;
                                       ...
                                       module Traversable:
                                         {
                                           ...
                                           type applicative_t('a) =
                                               BsAbstract.Result.Traversable(E)(Applicative).applicative_t('a);
                                           let traverse:
                                             ('a => applicative_t('b),
                                             t('a)) => applicative_t(t('b));
                                           let sequence:
                                             t(applicative_t('a)) =>
                                             applicative_t(t('a));
                                         };
                                       let traverse:
                                         ('a => Traversable.applicative_t('b),
                                         Traversable.t('a)) =>
                                         Traversable.applicative_t(Traversable.t('b));
                                       let sequence:
                                         Traversable.t(Traversable.applicative_t('a)) =>
                                         Traversable.applicative_t(Traversable.t('a));
                                       module Bitraversable:
                                         {
                                           ...
                                           let bitraverse:
                                             ...
                                             'c => Applicative.t('d),
                                             Belt.Result.t('a, 'c)) =>
                                             Applicative.t(Belt.Result.t(
                                                           'b, 'd));
                                           let bisequence:
                                             Belt.Result.t(Applicative.t('a),
                                                            Applicative.t('b)) =>
                                             Applicative.t(Belt.Result.t(
                                                           'a, 'b));
                                         };
                                       let bitraverse:
                                         ('a =>
                                          Bitraversable.applicative_t('b),
                                         'c =>
                                         Bitraversable.applicative_t('d),
                                         Bitraversable.t('a, 'c)) =>
                                         Bitraversable.applicative_t(
                                         Bitraversable.t('b, 'd));
                                       let bisequence:
                                         Bitraversable.t(Bitraversable.applicative_t('a),
                                                          Bitraversable.applicative_t('b)) =>
                                         Bitraversable.applicative_t(
                                         Bitraversable.t('a, 'b));
