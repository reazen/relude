...
let rec flip: 'a 'e. t('a, 'e) => t('e, 'a) =
  ioA => {
    ...
    | Async(onDoneA) =>
      Async(onDone => onDoneA(resultA => onDone(resultA |> Result.flip)))
  ...
  (r0ToA, ioR0) =>
    ...
    | Async(onDoneR0) =>
      Async(
        onDone =>
          onDoneR0(resultR0 =>
            onDone(resultR0 |> Result.map(r0ToA) |> Result.flip)
          ),
      )
...
and summonErrorMap:
  'a 'r0 'e.
  ('r0 => 'a, t('r0, 'e)) => t(Result.t('a, 'e), Void.t)
 =
  (r0ToA, ioR0) => {
    switch (ioR0) {
