...
describe("NonEmpty.List", () => {
  test("concat with two full NonEmpty.Lists", () => {
    ...
    expect(NonEmpty.List.concat(l1, l2)) |> toEqual(NonEmpty.List.make(1, [2, 3, 4, 5]));
  });

  test("foldLeft", () => {
    let ne = NonEmpty.List.make(1, [2, 3, 4, 5]);
    let result = NonEmpty.List.foldLeft((acc, curr) => Belt.List.concat(acc, [curr]), [], ne);
    expect(result) |> toEqual([1, 2, 3, 4, 5]);
  });

  test("foldRight", () => {
    let ne = NonEmpty.List.make(1, [2, 3, 4, 5]);
    let result = NonEmpty.List.foldRight((curr, acc) => Belt.List.concat(acc, [curr]), [], ne);
